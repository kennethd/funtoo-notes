# 2015-11-10 kld. i will try to keep the compilation output in a separate window, but document what i install with comments, like so;
# emerge app-misc/screen htop sys-process/lsof net-misc/wget net-misc/curl vim
...
# something like 22 packages were rebuilt.
# next, check for newly obsoleted packages
...
kennethd ~ # emerge --update --pretend  app-admin/webmin www-apps/postfixadmin mail-client/roundcube www-misc/awstats dev-db/postgresql dev-db/mysql net-mail/courier-imap net-libs/courier-authlib dev-libs/cyrus-sasl mail-filter/amavisd-new mail-filter/spamassassin app-antivirus/clamav mail-mta/postfix mail-filter/gld
...
# 12 packages are built or re-built
...
# Make sure Apache & PHP are working OK
...
# APACHE2_OPTS="... -D SECURITY -D PHP5"
...
# ok, one more try.  re-add geoip to USE in /etc/portage/make.conf
